...
import {deletePizzas, removePizza, plusElemCart, minusElemCart} from '../redux/actions/cart';
...
function Cart () {
	...
	const onPlusElemCart = (id)=> {
		dispatch(plusElemCart(id));
	}
	
	const onMinusElemCart = (id)=> {
		dispatch(minusElemCart(id));
	}
		...
		{totalCount ? 
		 ...
		 <div className="cart">
 ...
 
			{
				addedPizza.map(obj => {
					return <CartItem 
					onRemovePizza={()=> dispatch(removePizza(obj.id))}
					id={obj.id}
					name={obj.name} 
					type={obj.type} 
					size={obj.size} 
					...
					onPlusElemCart={onPlusElemCart}
					onMinusElemCart={onMinusElemCart}
					/>
				})
			}
